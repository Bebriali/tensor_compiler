...
struct Node_t {
    ...
    inline static Node_t* createOp(const std::string& op, Node_t* left, Node_t* right) const;
};
...
inline static Node_t* Node_t::createOp(const std::string& op, Node_t* left, Node_t* right) {
    Node_t* node = new Node_t(TokenType::OP, op);
    node->addChild(left);
    node->addChild(right);
    return node;
}
